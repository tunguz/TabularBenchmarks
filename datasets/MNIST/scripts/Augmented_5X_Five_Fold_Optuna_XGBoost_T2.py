...
def main():
    ...
    with LocalCUDACluster(n_workers=2) as cluster:
        ...
        logger.addHandler(logging.FileHandler("optuna_xgb_output_1_5x.log", mode="w"))

        optuna.logging.enable_propagation()  # Propagate logs to the root logger.
        optuna.logging.disable_default_handler()  # Stop showing logs in sys.stderr.

        study = optuna.load_study(storage="sqlite:///xgb_optuna_mnist_0_5x.db",
                                    study_name="five_fold_optuna_xgb_0_5x")
        logger.info("Start optimization.")
        #study.optimize(objective, n_trials=2)
        study.optimize(lambda trial: objective(client, train, target, train_folds, val_folds, train_ys, val_ys, trial), n_trials=40)

    df = study.trials_dataframe(attrs=('number', 'value', 'params', 'state'))
    df.to_csv('optuna_xgb_output_1_5x.csv', index=False)
